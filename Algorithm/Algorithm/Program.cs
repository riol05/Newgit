...
namespace _00._Algorithm
{
    internal class Program
    {
        ...
        // 3. 명확성 : 수행 과정은 모호하지 않고 명확하게, 정확한 수단을 제공해야 함
        // 4. 유한성 : 수행 과정은 무한하지 않고 유한한 작업 이후에 정지해야 함
        // 5. 효과성 : 모든 과정은 명백하게 실행 가능해야 함(불가사의한 문제를 가져와서 풀어봐라 하면 못함.효과성 위반)
        int FindIndex1(int[] array, int value)
        {
            ...
            return -1;
        } // 갖고 있는 배열중에 원하는 값을 찾아줄때 사용하는 알고리즘이라고 보면 된다.
         ...
         * 자료구조 (DataStructure)
         * 
         * 자료들을 어떻게 저장하는지에 따라서 효율 차이가 나게된다. 데이터를 어떻게 저장하는지에 대해 연구한 학문
         * 프로그래밍에서 데이터를 효율적인 접근 및 수정을 가능케 하는 자료의 조직, 관리, 저장을 의미
         * 데이터 값의 모임, 또 데이터 간의 관계, 그리고 데이터에 적용할 수 있는 함수나 명령을 의미
         **************************************************************************************/

        // <자료구조 형태>
        // 선형구조 : 자료 간 관계가 1 대 1인 구조 .(여러가지 자료 중에 내가 원하는 위치에 있는 자료를 즉각적으로 확인할수있는 자료)
        //          배열, 연결리스트, 스택, 큐, 덱
         ...
         * 상황에 따라 적합한 알고리즘을 선택할 수 있도록 하는 기준을 판단하기위해 알고리즘의 성능 판단 기능이 필요하다.
         ********************************************************************/

        // <알고리즘 평가 기준>
        // 컴퓨터에서 알고리즘과 자료구조의 평가는 시간과 공간 두 자원을 얼마나 소모하는지가 효율성의 중점
        // 일반적으로 시간을 위해 공간이 희생되는 경우가 많음
        // 일반적으로 이중에 시간 복잡도를 공간보다 좀더 중요시하는 경향이 있다.
        // 메모리의 발전 속도에 비하면 처리속도(CPU)의 발전속도는 조금 더딘편이 있기 때문에 시간 복잡도를 중요시 하는것.
        ...
        // 입력값       Case1	    Case2   	   Case3        Case4
        // n = 1            1           1              1            2
        // n = 10           1          10            100           20
        // n = 100          1         100         10,000          200  
        // n = 1000         1        1000      1,000,000         2000
        // Big-O		 O(1)	     O(n)   	   O(n²)        0(n)// 최고 차항만 신경쓰기 때문에 0(2n)이나 0(n+10) 할수 없다. 제곱수만 받아줌1
        ...
        // 최선          평균          최악
        // O(1)    O(n/2) => O(n)     O(n)
        //최선의 시간은 고려 대상이 아니다. 평균과 최악의 상황으로 고려를 판단한다.
