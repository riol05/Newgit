namespace _06.UserDefineType
{
        internal class Program
        {
            ...
            // <열거형 기본사용>
            // enum 열거형이름 { 멤버이름, 멤버이름, ... } //C#에선 enum class 가 아닌 그냥 enum만 써줘도 
            ...
            enum Day
            {
                None = 0b_0000_0000,        // 0 // _언더바는 자리수 표시형
             ...
             * 교수님이 안써줬으면 좋겠다 하심 // 가능하면 구조체를 만드는게 좋다. 간결하게 정리할수 있는 자료형
             *튜플은 서로 비교할때 순서만 달라고 다르게 받아버린다.
             ****************************************************************/
             ...
             * 튜플보다 구조체를 쓰는게 헷갈리지 않는다.
             ****************************************************************/

            // <구조체 구성>
            // struct 구조체 이름 {구조체 내용}
            // 구조체 내용으로는 변수와 함수가 포함 가능
            // 구조체에는 접근 제한자를 다 붙여줘야한다.
            // public int나 public void, private void 식으로 다 붙여줘야함 // C#은 실수를 용납하지 않기 때문이다. 생성자도 붙여줘야함
            struct Student
            {
                public string name;
                public int math;
            ...
            struct Player
            {
                ...
                // C# 10.0 이후 버전 : 기본 초기화를 추가하지 않는 경우 기본 초기화는 자동으로 생성됨 // 초기화가 자동
                /*public Player()
                {
                    // 변수를 기본값으로 초기화 
                }*/
                ...
                public Player(string name, int hp)
                {
                    ...
                    this.name = name;   // 값을 모두 세팅해줘야한다.
                    this.hp = hp;       // this : 자기 자신을 가리킴
                    this.mp = 0;        // this : 구조체에 있는 멤버 변수 이름을 까먹으면 오류로 나타난다.
                }

                public Player(string name, int hp, int mp) : this(name, hp) // 이전 초기화를 이용하여 초기화 간소화  //생성자 위임
                {
                    this.mp = mp;
                }
            }
