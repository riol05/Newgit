...
public:
    void Use() override         // 오버라이딩 : 가상함수를 재정의하여 구체화
    {
        cout << "폭탄을 사용하여 폭발을 일으킵니다." << endl;
    }
};

void AbstractionMain1()
{
...
{
public:
    virtual void Cry() = 0;       // 동물의 울음소리 부모클래스에서 정의하기 어려움
};
...
void AbstractionMain2()
{
    ...
    // //부모 클래스를 가상함수를 지정해놓지 않거나
    //override를 쓰지 않았기 때문에, 순수 가상 함수를 만들어 놓지 않았기 때문에
}